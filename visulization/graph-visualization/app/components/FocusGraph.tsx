...
function FocusGraph({data, enableDelay=4000}: { data: string, enableDelay?: number }) {
    ...
    let parsedData: { nodes: any[], links: any[] }
    try {
        parsedData = JSON.parse(data)
        const randomizedNodes = parsedData['nodes'].map((node: any) =>
            typeof node === 'object' && node !== null
                ? { ...node, group: Math.floor(Math.random() * 7) + 1 }
                : { group: Math.floor(Math.random() * 7) + 1 }
        );
        parsedData['nodes'] = randomizedNodes;
        console.debug("Parsed data nodes:", parsedData['nodes'])
    } catch (err) {
        let errorMessage
    ...
    return <div className="bg-background text-foreground">
        {Graph}
        <div className="absolute top-[5px] right-[5px] flex gap-4">
            <button id="axesToggle" className="bg-gray-800 hover:bg-gray-900 text-white font-bold py-2 px-4 rounded" onClick={handleAxesClick}>
                {(areAxesVisible ? 'Hide' : 'Show')} Axes
            </button>
            <button id="resetToggle" className="bg-gray-800 hover:bg-gray-900 text-white font-bold py-2 px-4 rounded" onClick={handleResetClick}>
                Reset Camera
            </button>
            <button id="rotationToggle" className="bg-gray-800 hover:bg-gray-900 text-white font-bold py-2 px-4 rounded" onClick={handleRotationClick}>
